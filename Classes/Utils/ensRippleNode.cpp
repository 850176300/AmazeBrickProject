...
void CrippleSprite::doTouch(const Vec2&touchPoint,float depth,float r){
...
void CrippleSprite::onDraw(const Mat4 &transform, uint32_t flags){
  ...
  //use scissor test is more fast than clippingNode for large rect.
	...
	Rect rect_world=RectApplyAffineTransform(rect_local, this->nodeToWorldTransform());
...
void CrippleSprite::onDrawDebug(const Mat4 &transform, uint32_t flags){
    ...
    for(int i=0;i<nIDtri;i++){
        ...
        Vec2 vertices[]={M4TransV2(m,Vec2(v0.x(),v0.y())),M4TransV2(m,Vec2(v1.x(),v1.y())),M4TransV2(m,Vec2(v2.x(),v2.y()))};
        DrawPrimitives::drawPoly(vertices, 3, true);
