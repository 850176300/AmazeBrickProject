...
float PNDisPointToLine(const Vec2&point,const Vec2&start,const Vec2&end){
    ...
    Vec2 sp=point-start;
    Vec2 se=end-start;
    Vec2 se_e=se.getNormalized();
    float PNDis=sp.cross(se_e);//ccpCross(sp, se_e);
...
{
    float Len_v0v1=v0.getDistance(v1);
    float Len_v1v2=v2.getDistance(v2);
    float Len_v2v0=v2.getDistance(v0);
    float a=Len_v0v1;
    float b=Len_v1v2;
    float c=Len_v2v0;
    float temp=2*(a+b+c);
...
//otherwise the return vector will contain three element (p0,p1,p2)
{
    ...
    for(int i=0;i<3;i++){
        ...
        Vec2 dir_vvn=(vn-v).getNormalized();
        Vec2 dir_vvf=(vf - v).getNormalized();
        Vec2 dir_vp=(dir_vvn+dir_vvf).getNormalized();//direction of vp
        float cos_vnvvf=dir_vvn.dot(dir_vvf);//ccpDot(, dir_vvf);
        float sin_halfvnvvf=sqrtf(MAX(0,0.5*(1-cos_vnvvf)));//sin(0.5*angle_vnvvf)
        float len_vp=d/sin_halfvnvvf;//length of vp
        Vec2 vp=dir_vp*len_vp;//ccpMult(dir_vp, len_vp);
        Vec2 p=v+vp;//got p
        plist.push_back(p);
...
//if point in circle, then return empty vector
{
    ...
    tangentPointLeft=point + dirPTLeft * disPT;//ccpMult(dirPTLeft, disPT);
    
    tangentPointRight=point + dirPTRight * disPT;//ccpMult(dirPTRight, disPT);
    vector<Vec2> rs;
    rs.push_back(tangentPointLeft);
    rs.push_back(tangentPointRight);
    return rs;
